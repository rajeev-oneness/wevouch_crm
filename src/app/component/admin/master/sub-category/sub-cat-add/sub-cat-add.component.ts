...
@Component({
  selector: 'app-sub-cat-add',
  templateUrl: './sub-cat-add.component.html',
  styleUrls: ['./sub-cat-add.component.css']
})
export class SubCatAddComponent implements OnInit {
  ...
  subCategoryFormSubmit(formData){
    ...
    if( formData?.valid ){
      ...
      this._loader.startLoader('loader');
      this._api.subCategoryCreate(mainForm).subscribe(
        res => {
          console.log(res);
